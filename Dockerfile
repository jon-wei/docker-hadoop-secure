# Creates pseudo distributed kerberized hadoop 2.8.5
#
# docker build -t kerbhadoop .
...
RUN yum clean all; \
    ...
    glibc-headers gcc-c++ cmake git zlib-devel && yum clean all && yum update -y libselinux && yum clean all
...
#RUN curl -LOH 'Cookie: oraclelicense=accept-securebackup-cookie' 'http://download.oracle.com/otn-pub/java/jdk/8u271-b09/61ae65e088624f5aaa0b1d2d801acb16/jdk-8u271-linux-x64.rpm'
#RUN wget --no-check-certificate --no-cookies --header 'Cookie: oraclelicense=accept-securebackup-cookie' 'http://download.oracle.com/otn-pub/java/jdk/8u271-b09/61ae65e088624f5aaa0b1d2d801acb16/jdk-8u271-linux-x64.rpm'
COPY local_files/jdk-8u271-linux-x64.rpm /

RUN rpm -i jdk-8u271-linux-x64.rpm
RUN rm jdk-8u271-linux-x64.rpm
...
# Kerberos client
RUN yum clean all && rpm --rebuilddb && yum install krb5-libs krb5-workstation krb5-auth-dialog -y && yum clean all
...
COPY local_files/hadoop-2.8.5.tar.gz $HADOOP_PREFIX-2.8.5.tar.gz
RUN tar -xzvf $HADOOP_PREFIX-2.8.5.tar.gz -C /usr/local
RUN cd /usr/local \
...
#RUN curl -L https://archive.apache.org/dist/hadoop/core/hadoop-2.8.5/hadoop-2.8.5-src.tar.gz | tar -xz -C /tmp
COPY local_files/hadoop-2.8.5-src.tar.gz /tmp/hadoop-2.8.5-src.tar.gz
RUN tar -xzf /tmp/hadoop-2.8.5-src.tar.gz -C /tmp

# build native hadoop-common libs to remove warnings because of 64 bit OS
RUN rm -rf $HADOOP_PREFIX/lib/native
RUN cd /tmp/hadoop-2.8.5-src/hadoop-common-project/hadoop-common \
    && mvn compile -Pnative \
...
RUN cd /tmp/hadoop-2.8.5-src/hadoop-yarn-project/hadoop-yarn/hadoop-yarn-server/hadoop-yarn-server-nodemanager \
    && mvn compile -Pnative \
